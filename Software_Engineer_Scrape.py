...
try:
    ...
    for loc in locations:
        for pageNum in range(0,100):
            urls.append('https://www.indeed.com/jobs?q=software%20engineer&l={}&start={}'.format(loc,str(pageNum*10)))
      
    #print(urls)
    
    #open the file to write job desc,job title
    fw=open('NEW_SE.csv','w',encoding='utf8')
    ...
    for url in urls:
        ...
        for job in jobs:
           ...
           writer.writerow([jd,'Software Engineer']) 
           count=count+1
           print(count)
        page=page+1
        print("page",page)
